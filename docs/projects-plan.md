...
| 4 | **Session** ✅ — `project_conversation()` opens one per project on its first tick; `record_project_tick()` resets the context and files the turn (conductor's briefing as the ask, worker's summary as the reply). Transcript grows, context does not. Read-only: nothing routes typing into it. The 30-day chat reap now spares a living project's thread. |
...
| 6 | **15m default** ✅. `POST /projects/{id}/tick` forces one — a request, not a preemption: it raises a flag the tick clears at its *start*, so a force raised while one runs survives it. Editing the goal and the period were already `PATCH`. The conductor can slow its own heartbeat with `project_pace` — and **only** slow it: the interval a person set is the fastest it may wake, so the one direction a model can move the cost is down. Editing the period by hand clears any backoff. |
